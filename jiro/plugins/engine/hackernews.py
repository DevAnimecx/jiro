...
class HackerNewsEnginePlugin(BaseEnginePlugin):
    ...
    async def get_top_stories(self, limit: int=25) -> List[Dict[str, Any]]:
        ...
        ids = await self.client.get(url, engine=self.name)
    ...
    def _parse_algolia_results(self, hits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for hit in hits:
            ...
            result = {'title': hit.get('title', '') or hit.get('story_title', '') or '', 'link': hit.get('url', '') or f"{self.HN_URL}/item?id={hit.get('objectID', '')}", 'snippet': hit.get('story_text', '')[:300] if hit.get('story_text') else hit.get('comment_text', '')[:300] if hit.get('comment_text') else '', 'author': hit.get('author', ''), 'points': hit.get('points', 0), 'num_comments': hit.get('num_comments', 0), 'created_at': hit.get('created_at', ''), 'object_id': hit.get('objectID', ''), 'type': item_type, 'source': 'hackernews'}
    ...
    async def _get_items_batch(self, ids: List[int]) -> List[Dict[str, Any]]:
        ...
        import asyncio
        tasks = [self.client.get(f'{self.BASE_URL}/item/{id}.json', engine=self.name) for id in ids]
    ...
    def _normalize_item(self, data: Dict[str, Any], url: str) -> Dict[str, Any]:
        ...
        return {'title': data.get('title', '') or data.get('text', '')[:100] or '', 'link': url, 'snippet': data.get('text', '')[:300] if data.get('text') else '', 'author': data.get('by', ''), 'points': data.get('score', 0), 'num_comments': len(data.get('kids', [])), 'created_at': data.get('time', 0), 'type': item_type, 'source': 'hackernews', 'item_id': data.get('id')}
