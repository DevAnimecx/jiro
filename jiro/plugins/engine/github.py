...
class GitHubPlugin(BaseEnginePlugin):
    ...
    rate_limit_rpm = 30  # GitHub API limits
    ...
    async def search(self, query: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        search_type = kwargs.get("type", "code")  # code, repositories, issues, users, commits, topics
        ...
        params = {
            "q": search_query,
            "per_page": min(kwargs.get("max_results", 25), 100),
            "page": (kwargs.get("start", 0) // 100) + 1,
    ...
    def _parse_code(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "path": item.get("path", ""),
            "language": self._guess_language(item.get("path", "")),
            "source": "github",
            "type": "code",
        }
    ...
    def _parse_user(self, item: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "link": item.get("html_url", ""),
            "snippet": item.get("bio", "")[:300] if item.get("bio") else "",
            "type": "user" if item.get("type") == "User" else "organization",
